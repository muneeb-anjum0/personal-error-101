...
Phase 5 uses the official GitHub REST API. Anonymous mode fetches public repositories for `GITHUB_USERNAME`; token mode uses server-side `GITHUB_TOKEN` and may include private repositories only when `GITHUB_INCLUDE_PRIVATE=true` and the token has sufficient access.
